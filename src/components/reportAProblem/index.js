...
class ReportAProblemController {
  ...
  $onInit() {
    ...
    let servicesWatcher = self.$scope.$watch(
      () => {
        ...
        let servicesLoaded =
          self.parentCtrl.fullViewService.servicesArray !== undefined;
        let calculatePrimaViewItDone =
          self.parentCtrl.fullViewService.calculatePrimaViewItDone();
        let calculatePcDeliveryDone =
          self.parentCtrl.fullViewService.calculatePcDeliveryDone;
        let calculateSvcIdDone =
          self.parentCtrl.fullViewService.calculateSvcIdDone;
        let domElement = !!self.$element.parent().parent().find("h4").length;
        return (
          services &&
          servicesLoaded &&
          calculatePrimaViewItDone &&
          calculatePcDeliveryDone &&
          calculateSvcIdDone &&
          domElement
        );
      },
  ...
  showReportAProblembutton() {
    ...
    self.$element
      ...
      .parent()
      .find("h4")
      .after(self.$compile(reportAProblemHTML)(self.$scope));
        ...
        controller: function ($scope, $mdDialog) {
          ...
          $scope.sendReport = function (answer) {
            let data = {
              ...
              source: recordData.pnx.display.source
                ? recordData.pnx.display.source[0]
                : "unknown",
              index: 0,
              page: 0,
              scope: "",
            ...
            if (recordData.pnx.control.originalsourceid !== undefined) {
              data["originalsourceid"] =
                recordData.pnx.control.originalsourceid[0];
            }

            if (
              $scope.report.replyTo.length > 0 &&
              $scope.report.message.length > 0
            ) {
              $mdDialog.hide();
              ...
              self
                ...
                .then(
                  function (response) {
                    let message =
                      self.$translate.instant(
                        "nui.customization.report_a_problem.success"
                      ) || "Thank you for your feedback!";
                    self.MessageService.show(message, {
                      scope: $scope,
                      hideDelay: 5000,
                    });
                  },
                  function (response) {
                    let message =
                      self.$translate.instant(
                        "nui.customization.report_a_problem.fail"
...
ReportAProblemController.$inject = [
  "$element",
  "$compile",
  "$scope",
  "$mdDialog",
